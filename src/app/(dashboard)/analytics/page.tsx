...
export default function AnalyticsPage() {
    ...
    useEffect(() => {
        async function load() {
            ...
            const now = new Date()
            const weekAgoTime = subDays(now, 7).getTime()

            let myTotalTasksCount = 0
            let myCompletedCount = 0
            let myCompletedThisWeekCount = 0
            let partnerCompletedThisWeekCount = 0

            const hourCounts: Record<number, number> = {}

            const weekBoundaries = Array.from({ length: 7 }, (_, i) => {
            ...
            for (let i = 0; i < allTasks.length; i++) {
                ...
                const isPartner = profile?.partner_id && t.assignee_id === profile.partner_id
                ...
                if (isMine) {
                    ...
                    if (t.is_completed) {
                        ...
                        if (t.completed_at) {
                            ...
                            if (completedTime >= weekAgoTime) {
                                myCompletedThisWeekCount++
                            }
                            const h = completedDate.getHours()
                    ...
                    if (t.created_at) {
                        ...
                        for (let j = 0; j < 7; j++) {
                            const wb = weekBoundaries[j]
                            if (createdTime >= wb.start && createdTime <= wb.end) {
                                wb.total++
                                if (t.is_completed) {
                                    wb.completed++
                                }
            ...
            const bestDay = weekBoundaries.reduce(
                (best, d) => (!best || d.completed > best.count) ? { label: d.label, count: d.completed } : best,
                null as { label: string; count: number } | null
            )

            const rate = myTotalTasksCount > 0 ? Math.round((myCompletedCount / myTotalTasksCount) * 100) : 0

            const weekData = weekBoundaries.map(wb => ({
                date: wb.date,
                label: wb.label,
            ...
            setAnalytics({
                totalCompletedAllTime: myCompletedCount,
                completedThisWeek: myCompletedThisWeekCount,
