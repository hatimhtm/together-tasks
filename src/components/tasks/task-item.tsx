...
const TYPOGRAPHY = {
  labelBase: "font-label font-bold",
  labelSm: "text-[11px] uppercase tracking-wider",
  labelXs: "text-[10px] uppercase tracking-widest",
};
...
const getPriorityBadgeStyles = (priority: string) => {
  switch (priority) {
    ...
    default:
      return "text-tertiary-fixed-dim bg-tertiary-fixed/10";
  }
};
...
const getCheckboxBorder = (priority: string) => {
  switch (priority) {
    case "urgent":
      return "border-error hover:border-error hover:bg-error/10";
    case "high":
      return "border-secondary hover:border-secondary hover:bg-secondary/10";
    case "low":
      return "border-primary hover:border-primary hover:bg-primary/10";
    default:
      return "border-outline-variant hover:border-primary hover:bg-primary/10";
  }
};
...
export const TaskItem = forwardRef<HTMLDivElement, TaskItemProps>(
  (
    ...
    const hasMetadata =
      ...
      task.scope === "shared" ||
      task.emergency_level === "high" ||
      task.emergency_level === "critical" ||
      task.importance_level === "high" ||
      task.importance_level === "critical";
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-between px-5 pointer-events-none rounded-xl">
          ...
          <motion.div
            ...
            className="h-8 w-8 rounded-full bg-error/20 flex items-center justify-center"
          >
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-1.5 pl-1 border-l-2 border-primary/20">
                      <p
                        className={cn(
                          ...
                          "text-[10px] text-primary/70 uppercase tracking-wider",
                        )}
                      >
                        Steps
                      </p>
                  ...
                  <div className="flex gap-2 justify-end">
                    <button
                      ...
                      className="text-xs px-3 py-1.5 rounded-lg text-on-surface-variant hover:bg-surface-container-highest transition-colors font-label font-medium"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className={cn(
                        TYPOGRAPHY.labelBase,
                        "text-xs px-4 py-1.5 bg-primary text-on-primary rounded-lg hover:shadow-lg hover:shadow-primary/20 transition-all",
                      )}
                    >
                      Save
                    </button>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      task.scope === "shared" &&
                        task.completed_by?.includes(userId) &&
                        !task.is_completed &&
                        "bg-primary/20 border-primary shadow-[0_0_10px_rgba(255,183,125,0.3)]",
                    )}
                    title={
                      task.scope === "shared"
                        ? task.completed_by?.includes(userId)
                          ? "Waiting for partner…"
                          : task.completed_by?.length === 1
                            ? "Partner finished! Your turn."
                            : "Mark complete"
                        : "Complete"
                    }
                  >
                      ...
                      <span
                        className={cn(
                          ...
                          task.priority === "urgent"
                            ? "text-error opacity-0 group-hover/check:opacity-100"
                            : task.priority === "high"
                              ? "text-secondary opacity-0 group-hover/check:opacity-100"
                        ...
                        {task.due_date && (
                          <span
                            className={cn(
                              ...
                              isOverdue
                                ? "text-error bg-error/10 font-bold"
                    ...
                    {isExpanded && (
                      <div className="mt-4 space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                          <div
                            className="relative inline-flex items-center gap-2 px-3 py-1.5 bg-surface-container-high rounded-full border border-outline-variant/20"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <div
                              className={cn(
                            ...
                            {task.subtasks.map((st: any, i: number) => {
                              if (typeof st === "string")
                                return (
                                  <div
                                    key={i}
                                    className="flex items-center gap-3 text-sm font-body text-on-surface-variant pl-1"
                                  >
                              ...
                              return (
                                <label
                                  key={st.id}
                                  className="flex items-center gap-3 text-[13px] font-body cursor-pointer group/st bg-surface-container-low/50 hover:bg-surface-container-low px-2 py-1.5 rounded-lg transition-colors"
                                >
                                  <div className="relative flex items-center justify-center shrink-0">
                                    ...
                                    />
                                    <span
                                      className="material-symbols-outlined text-[14px] absolute text-on-primary opacity-0 peer-checked:opacity-100 pointer-events-none font-bold"
                                      style={{
                                  ...
                                  <span
                                    className={cn(
                                      ...
                                      st.is_completed
                                        ...
                                        : "text-on-surface group-hover/st:text-on-surface font-medium",
                                    )}
                                  >
                                    {st.title}
                                  </span>
